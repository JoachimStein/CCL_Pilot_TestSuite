describe("Access_Right_Foreign_Purchase_Special_Notesheet_Based", function () {
    const pr_user_ids = [{
        userName: "init.user",
        role: "PR Initiator"
    },
    ...
    pr_user_ids.forEach((user, index) => {
        it(`User for Purchase Requisition : ${user.role}`, () => {
            ...
            if (userIndex === 0) {
                ...
                cy.wait(1000)
                cy.get("table.o_list_table>tbody>tr:first-child>td:first-child").type("SW Cement 97% Pure").type('{Enter}') // Product
                cy.wait(1000)
